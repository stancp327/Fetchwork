...
const PostJob = () => {
  ...
  // Reactive zip lookup ΓÇö auto-fills city/state when user enters a valid zip
  ...
  const handleGenerateDescription = async () => {
    ...
    try {
      ...
      setAiMsg(result.aiGenerated ? 'Γ£¿ AI-generated ΓÇö feel free to edit.' : '≡ƒô¥ Template applied ΓÇö edit to make it yours.');
      setTimeout(() => setAiMsg(''), 6000);
    } catch (err) {
      setAiMsg('Generation failed ΓÇö try again.');
      setTimeout(() => setAiMsg(''), 4000);
    } finally {
  ...
  const applyTemplate = async (templateId) => {
    ...
    try {
      ...
      setTemplateMsg(`Template "${t.name}" applied Γ£à`);
  ...
  const handleSaveTemplate = async () => {
    ...
    try {
      ...
      setTemplateMsg('Template saved Γ£à');
      setSaveTemplateName('');
  ...
  if (success) {
    return (
      <div className="post-job-success">
        <h3>Γ£à Job Posted Successfully!</h3>
        ...
        {canTemplates && templates.length > 0 && (
          <div className="pj-template-bar">
            <div className="pj-template-header">
              <span className="pj-template-icon">≡ƒôï</span>
            ...
            <div className="pj-template-list">
              {templates.map(t => (
                <div key={t._id} className="pj-template-chip">
                  ...
                  <button className="pj-template-delete" onClick={() => handleDeleteTemplate(t._id)} title="Delete">Γ£ò</button>
                </div>
              ))}
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit}>
                ...
                <div className="pj-ai-title-wrap">
                  {titleSuggestion ? (
                    <div className="pj-ai-title-suggestion">
                      <span className="pj-ai-title-improved">Γ£¿ {titleSuggestion.improved}</span>
                    ...
                    <button type="button" className="pj-ai-inline-btn" disabled={titleFixLoading} onClick={async () => {
                      ...
                      {titleFixLoading ? 'Γ£¿ CheckingΓÇª' : 'Γ£¿ Improve title'}
                    </button>
                  )}
                </div>
              )}
            ...
            <div className="form-group">
              <div className="pj-label-row">
                ...
                <div className="pj-ai-btn-group">
                  ...
                  >
                    {aiGenerating ? 'ΓÅ│ GeneratingΓÇª' : canAIDescription ? 'Γ£¿ Write for me' : '≡ƒöÆ Write for me ┬╖ Plus'}
                  </button>
                  {formData.description.trim().length >= 20 && (
                    ...
                    >
                      {scopeExpanding ? 'Γ£¿ ExpandingΓÇª' : 'Γ£¿ Expand scope'}
                    </button>
                  )}
                </div>
              </div>
              ...
              <textarea
                ...
                onChange={handleInputChange}
                placeholder="Describe your project in detail, or click Γ£¿ Write for me aboveΓÇª"
                rows={6}
                maxLength={5000}
              />
            ...
            <div className="form-group">
              <div className="pj-ai-cat-label-row">
                ...
                {formData.title && (
                  ...
                  >
                    {catDetecting ? 'Γ£¿ DetectingΓÇª' : 'Γ£¿ Auto-detect category'}
                  </button>
                )}
              </div>
              {catDetected && catDetected.confidence > 0.7 && (
                <div className="pj-ai-cat-banner">Γ£¿ Detected: {catDetected.category.replace(/_/g, ' ')} ΓÇö {catDetected.reason}</div>
              )}
              <CategoryCombobox
            ...
            <div className="form-group">
              ...
              {(formData.title || formData.description) && (
                <div className="pj-ai-skills-wrap">
                  ...
                  >
                    {skillsLoading ? 'Γ£¿ SuggestingΓÇª' : 'Γ£¿ Suggest skills'}
                  </button>
          ...
          <div className="form-section">
            ...
            <div className="post-job-grid-3">
              ...
              <div className="pj-ai-budget-wrap">
                ...
                >
                  {budgetEstimateLoading ? 'Γ£¿ EstimatingΓÇª' : 'Γ£¿ AI Budget Estimate'}
                </button>
                {budgetEstimate && (
                  <div className="pj-ai-budget-result">
                    <span className="pj-ai-budget-range">
                      ${budgetEstimate.low?.toLocaleString()} ΓÇô ${budgetEstimate.high?.toLocaleString()} {budgetEstimate.type === 'hourly' ? '/hr' : 'total'}
                    </span>
          ...
          <div className="form-section">
            ...
            <div className="work-type-cards">
              {[
                { value: 'remote', icon: '≡ƒîÉ', label: 'Remote', desc: 'Done online, anywhere' },
                { value: 'local', icon: '≡ƒôì', label: 'Local / On-site', desc: 'Freelancer comes to you' },
                { value: 'hybrid', icon: '≡ƒöä', label: 'Hybrid', desc: 'Mix of remote + on-site' },
              ].map(opt => (
                <button
                  key={opt.value}
                  type="button"
            ...
            {formData.locationType === 'remote' && (
              <div className="location-info-banner">
                ≡ƒîÉ Great ΓÇö freelancers worldwide can apply. No location details needed.
              </div>
            )}
            ...
            {formData.locationType !== 'remote' && (
              <>
                <p className="field-hint" style={{ marginBottom: '0.75rem' }}>
                  ≡ƒôì Let freelancers know where the work takes place so they can assess travel.
                </p>
                <div className="post-job-grid-location">
                  ...
                  <div className="form-group">
                    ...
                    {zipLookup.result && <span className="zip-city-hint">≡ƒôì {zipLookup.result.city}, {zipLookup.result.state}</span>}
                  </div>
                </div>
              </>
            )}
          ...
          {/* Scheduling ΓÇö only for local/hybrid */}
          {formData.locationType !== 'remote' && (
            <div className="form-section">
              <div className="form-section-title">≡ƒôà Scheduling</div>
              <div className="form-group">
          ...
          <div className="post-job-checkbox">
            <input
              ...
              onChange={handleInputChange}
            />
            <label htmlFor="isUrgent">ΓÜí This is an urgent job</label>
          </div>
          ...
          <div className="post-job-checkbox">
            <input
              ...
              onChange={handleInputChange}
            />
            <label htmlFor="recurringEnabled">ΓÖ╗∩╕Å Make this a recurring job</label>
          </div>
            ...
            <div className="pj-save-template">
              {!showSaveTemplate ? (
                ...
                <div className="pj-save-template-form">
                  ...
                  <button type="button" className="pj-save-template-cancel" onClick={() => setShowSaveTemplate(false)}>Γ£ò</button>
                </div>
              )}
            </div>
          )}
